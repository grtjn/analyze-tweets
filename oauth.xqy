...
(:
  let $service :=
     <oa:service-provider realm="">
       <oa:request-token>
         <oa:uri>https://twitter.com/oauth/request_token</oa:uri>
         <oa:method>GET</oa:method>
       </oa:request-token>
       <oa:user-authorization>
         <oa:uri>https://twitter.com/oauth/authorize</oa:uri>
       </oa:user-authorization>
       <oa:user-authentication>
         <oa:uri>https://twitter.com/oauth/authenticate</oa:uri>
         <oa:additional-params>force_login=true</oa:additional-params>
       </oa:user-authentication>
       <oa:access-token>
         <oa:uri>https://twitter.com/oauth/access_token</oa:uri>
...
as element(oa:response)
{
  ...
  let $authheader := concat("OAuth realm=&quot;", $service/@realm, "&quot;, ",
   ...
   let $options  := <xh:options>
                      <xh:headers>
						<xh:User-Agent>Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0)</xh:User-Agent>
                        <xh:Authorization>{$authheader}</xh:Authorization>
						<xh:Accept>application/xml,plain/text</xh:Accept>
                      </xh:headers>
                      { $data }
					  <xh:timeout>5</xh:timeout>
                    </xh:options>
      ...
      { if (string($tokenreq[1]/xh:code) != "200")
        then
          (<oa:error>{$tokenreq[1]}</oa:error>,
           <oa:error-body>{if ($tokenreq[2]/binary()) then xdmp:describe($tokenreq[2]) else $tokenreq[2]}</oa:error-body>)
        else
          $tokenreq[2]
      }
    </oa:response>
